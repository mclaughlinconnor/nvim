...
return
  {
    ...
    s({trig = "sum", wordTrig = true}, fmta([[\sum^{<>}_{<>}]], {i(1, "upper"), i(2, "lower")}), { condition = tex.in_mathzone }),
    s({trig = "int", wordTrig = true}, fmta([[\int^{<>}_{<>} ]], {i(1, "upper"), i(2, "lower")}), { condition = tex.in_mathzone }),
    s(
      "ls",
      {
        t({[[\begin{]]}),
    ...
    s({trig = "ceil", wordTrig = true}, fmta([[\left\lceil <> \right\rceil ]], {i(1)}), { condition = tex.in_mathzone }),
    s({trig = "floor", wordTrig = true}, fmta([[\left\lfloor <> \right\rfloor ]], {i(1)}), { condition = tex.in_mathzone }),
  },
  {
    parse(
      "note",
    ...
    s({trig = [[((%d*)\?([A-Za-z]+)[_^]?{%d+})/]], regTrig = true}, d(1, fraction, {}), { condition = tex.in_mathzone }), -- 2\sigma_{2}
